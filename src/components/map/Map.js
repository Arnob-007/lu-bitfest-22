...
import { SET_BUS_ROUTE, SET_STOPPAGE } from "../../state/Constants";
...
const Map = () => {
	...
	const map = useMapEvents({
		click(e) {
			pageState.state === ADDING_NEW_STOPPAGE &&
			...
			if (add) {
				...
				dispatch({
					...
					payload: {
						...
						route: [
							...buses.find((bus) => bus.id === pageState.stateData.bus_id)
								.route,
							stoppage.id,
						],
					},
					...
					payload: {
						...
						route: [
							...buses
								...
								.route.filter((st_id) => st_id !== stoppage.id),
						],
					},
				});
			}
			...
			{pageState.state === EDITING_BUS && buses?.length > 0 && (
				<Polyline
					...
					positions={buses
						...
						?.route.map(
							(stoppage_id) =>
								stoppages.find((st) => st.id === stoppage_id).position
						)}
				/>
			)}
					...
					<Popup>
						<div className='px-2 py-4 min-w-[200px]'>
							...
							{pageState.state === EDITING_BUS &&
								...
								(buses
									.find((bus) => bus.id === pageState.stateData.bus_id)
									.route.indexOf(stoppage.id) === -1 ? (
									<Button
										className='mt-4'
										type='secondary'
										onClick={() => handleStoppageClick(stoppage, true)}
									>
										Add to selected bus
									</Button>
								) : (
									<Button
										className='mt-4'
										type='secondary'
										onClick={() => handleStoppageClick(stoppage, false)}
									>
